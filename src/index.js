import {
  ...
  containsDulcetElement,
  PLUGIN_NAME,
  hasDulcetElementTypeAnnotationReturn,
} from './util';
...
let importedTypes = {};
let suppress = false;
const SUPPRESS_STRING = 'no babel-plugin-flow-dulcet-proptypes';
...
module.exports = function flowDulcetPropTypes(babel) {
  ...
  const isFunctionalDulcetComponent = path => {
    ...
    if (hasDulcetElementTypeAnnotationReturn(path.node)) {
      return true;
    }
    if (containsDulcetElement(path.node)) {
      return true;
    }
    return false;
  };
  ...
  return {
    visitor: {
      ...
      ClassDeclaration(path) {
        ...
        const extendsDulcetComponent = superClass && superClass.type === 'MemberExpression'
        && superClass.object.name === 'Dulcet'
        ...
        if (!extendsDulcetComponent && !extendsComponent) {
          $debug('Found a class that isn\'t a dulcet component', superClass);
          return;
        }
          ...
          propTypesAst = t.callExpression(
            t.memberExpression(
              t.callExpression(
                ...
                [t.stringLiteral('@dulcetjs/prop-types')]
              ),
              t.identifier('shape'),
            ),
            [propTypesAst],
